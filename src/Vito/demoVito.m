...
%% Init stuff
function [warped_surface, landmarked_img, frontal_raw,hardsym_images]=demoVito(X,detector,filename,rect)
% load model
%clear all
...
bbox = rect %detect_faces_fp(X,model.VJdetector,1);
if length(bbox) > 0
bbox=bbox(1,:);
...
elseif strcmp(detector,'dlib')
        commandStr = ['python dlib_landmark_script.py ',filename,' ',num2str(rect(1)),' ',num2str(rect(2)),' ',num2str(rect(3)),' ',num2str(rect(4))];
...
maped1 = imcrop(uint8([mapped_img(:,1:75,:), mapped_img1(:,76:end,:)]),[1 51 size(mapped_img,2) size(mapped_img,1)-50]);
maped2 = imcrop(uint8([mapped_img1(:,1:75,:), mapped_img(:,76:end,:)]),[1 51 size(mapped_img,2) size(mapped_img,1)-50]);
maped3 = imcrop(uint8([mapped_img(:,1:76,:), mapped_img1(:,74:end,:)]),[1 51 size(mapped_img,2) size(mapped_img,1)-50]);
maped4 = imcrop(uint8([mapped_img1(:,1:76,:), mapped_img(:,76:end,:)]),[1 51 size(mapped_img,2) size(mapped_img,1)-50]);
maped5 = imcrop(uint8([mapped_img(:,1:77,:), mapped_img1(:,73:end,:)]),[1 51 size(mapped_img,2) size(mapped_img,1)-50]);
maped6 = imcrop(uint8([mapped_img1(:,1:77,:), mapped_img(:,73:end,:)]),[1 51 size(mapped_img,2) size(mapped_img,1)-50]);

hardsym_images = {maped1,maped2,maped3,maped4,maped5,maped6};
